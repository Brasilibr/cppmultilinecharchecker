...
CC=g++
CPPFLAGS=-fno-trapping-math -Wformat -Werror=multichar -Werror=overflow -pedantic -Werror=overlength-strings -O3 -std=c++17 -D_FORCE_INLINES -I./src/include -I/usr/include/mysql
LIBS =-lm -lmysqlclient -lpthread -lcrypto -lz -lssl #### -lcurl####
EXEC=validateMultiByte.exe
SRC_DIR := src
...
validateMultiByte.exe: $(OBJ) $(OBJC) $(LIB_OBJ_FULL) $(LIB_OBJC_FULL) $(LEVEL2_OBJ_FULL) $(LEVEL2_OBJC_FULL)
# $@ = nome do case
# $^ = objects do case
	$(CC) -o $@ $^ $(LIBS) 
